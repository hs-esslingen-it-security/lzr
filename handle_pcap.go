...
func closeConnection(packet *packet_metadata, ipMeta *pState, write bool, ackingFirewall bool) {
...
func HandlePcap(opts *options, packet *packet_metadata, ipMeta *pState, timeoutQueue chan *packet_metadata,
	retransmitQueue chan *packet_metadata) {
	...
	if len(packet.Data) > 0 {
		...
		if ForceAllHandshakes() {
			handleExpired(opts, packet, ipMeta, timeoutQueue)
			return
		}

		packet.syncHandshakeNum(handshakeNum)

		closeConnection(packet, ipMeta, true, isHyperACKtive)
		return
	...
	if handshakeNum == 1 && HyperACKtiveFiltering() && !isHyperACKtive {
		...
		if ipMeta.getEphemeralRespNum(packet) > getNumFilters() {
			closeConnection(packet, ipMeta, true, true)
			return
		}
	}
	...
	if packet.SYN && packet.ACK {
		...
		if handshakeNum == 1 && HyperACKtiveFiltering() {
			...
			if isHyperACKtive {
				...
				closeConnection(packet, ipMeta, false, isHyperACKtive)
				return
			} else {
		...
		toACK := true
		toPUSH := false
		SendAck(opts, packet, ipMeta, timeoutQueue, retransmitQueue,
			toACK, toPUSH, ACK)
		return
	}
